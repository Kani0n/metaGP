...
def exec_humann(concat_file, config_file, nCores):
    ...
    cmd = 'humann --input ' + concat_file + ' --input-format fasta -o ' + concat_dir + ' --search-mode uniref90 --threads ' + 1
    cmd += ' --metaphlan-options="--index ' + bowtie_index + ' --bowtie2db ' + bowtie_db + ' --nproc ' + nCores + '" '
    cmd += '--nucleotide-database ' + nucleotide_db + ' --protein-database ' + protein_db + ' --bypass-nucleotide-index'
    print(cmd)
    os.system(cmd)


def run_functional_profiling(mapping_file, process_dir, nCores):
    ...
    for idx in df_mapping.index:
        ...
        output_dir = os.path.join(process_dir, sample)
